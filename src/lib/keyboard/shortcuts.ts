...
// import { KeyboardEvent } from 'react'; // Removed to use native Global KeyboardEvent
...
export class ShortcutUtils {
  ...
  static eventToShortcut(event: KeyboardEvent): string {
    ...
    // Safety check: if event.key is undefined, return empty shortcut
    if (!event.key) {
...
export class ShortcutManager {
  ...
  private hooks: Set<(action: ShortcutAction, event: KeyboardEvent) => void> = new Set();

  /**
   * Register a hook to be called when any shortcut is executed
   * Useful for macro recording or analytics
   */
  addHook(hook: (action: ShortcutAction, event: KeyboardEvent) => void): void {
    this.hooks.add(hook);
  }
   ...
   * Remove a hook
   */
  removeHook(hook: (action: ShortcutAction, event: KeyboardEvent) => void): void {
  ...
  handleKeyDown(event: KeyboardEvent): boolean {
    ...
    if (definition) {
      ...
      if (handlers && handlers.length > 0) {
        ...
        // Call hooks (Macro Recorder, Analytics, etc.)
        this.hooks.forEach(hook => hook(definition.action, event));
  ...
  /**
   * Manually execute an action (e.g. from Command Palette or Macro)
   */
  executeAction(action: ShortcutAction): boolean {
    const handlers = this.handlers.get(action);
    if (handlers && handlers.length > 0) {
