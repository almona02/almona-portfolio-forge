...
export class ProfileDefinitionManager {
  ...
  async createProfileFromDefinition(input: ProfileDefinitionInput): Promise<Profile> {
    ...
    const profileData = {
      ...
      code: input.profileCode,
      material: input.material,
      width: input.width,
      height: input.height,
      thickness: input.materialThickness,
      weight_per_meter: input.weightPerMeter,
      type: input.role,
      profile_role: input.role,
      default_k_factor_45: kFactor45,
      default_k_factor_90: kFactor90,
      cross_section_image_url: input.crossSectionImageUrl,
      data_sheet_url: input.dataSheetUrl,
      profile_annotations: input.annotations || {},
      stock_quantity: 0,
      min_stock_level: 0,
      is_active: true,
      is_archived: false,
