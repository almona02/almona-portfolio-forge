...
const Services = () => {
  ...
  }, [realTimeData, loadPredictiveData, updateRealTimeData]);
  ...
  const launchMaintenanceTicket = useCallback((maintenanceType: 'preventive' | 'corrective' | 'emergency') => {
  ...
  // Memoize status helpers to prevent recreation on every render
  const getStatusColor = useCallback((status: MachineHealth['status']) => {
    switch (status) {
      case 'optimal': return 'text-green-400';
      case 'degraded': return 'text-yellow-400';
      case 'maintenance_required': return 'text-amber-400';
      case 'critical': return 'text-red-400';
      default: return 'text-gray-400';
    }
  }, []);

  const getStatusIcon = useCallback((status: MachineHealth['status']) => {
    switch (status) {
  ...
  const handleViewModeChange = useCallback((mode: 'simple' | 'advanced') => {
    setViewMode(mode);
  }, []);
          ...
          {viewMode === 'simple' ? (
            ...
            <>
              ...
              <div className="mb-16 text-center">
                ...
                <h1 className="typography-h1 md:text-6xl mb-6 bg-gradient-to-r from-amber-400 via-amber-500 to-red-500 bg-clip-text text-transparent">
                  {t('services.title')}
                </h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                <Card className="bg-gradient-to-br from-gray-900 to-black border-amber-500/20">
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-amber-400">{predictiveAlerts.length}</div>
              ...
              <Tabs value={activeTab} onValueChange={handleTabChange}>
                <TabsList className="grid w-full grid-cols-1 sm:grid-cols-6 max-w-6xl mx-auto mb-12 bg-gradient-to-r from-gray-900 to-black border 500/20 p-2 rounded-xl card-glass-dark">
                  ...
                  <div className="space-y-8 fade-in-up">
                      ...
                      <CardContent>
                        <div className="space-y-4">
                          {predictiveAlerts.map((alert) => (
                            <div
                              key={alert.id}
                              className={`p-4 rounded-lg border fade-in-up ${alert.severity === 'critical' ? 'border-red-500/30 bg-red-500/10' :
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      <ServiceCard
                        ...
                        onActionClick={() => handleTabChange("predictive")}
                        highlight={true}
                      />
                      <ServiceCard
                        icon="bolt"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {machineHealth.map((machine) => (
                            <div
                              ...
                              className={`p-4 rounded-lg border transition-transform hover:scale-[1.02] ${machine.status === 'optimal' ? 'border-green-500/30' :
                                machine.status === 'degraded' ? 'border-yellow-500/30' :
                                  machine.status === 'maintenance_required' ? 'border-amber-500/30' :
                                    'border-red-500/30'
                ...
                <TabsContent value="service-kpis">
                  <div className="space-y-8 fade-in-up">
                    ...
                    {/* Full BI Dashboard */}
                    <div className="bg-gradient-to-br from-gray-900 to-black border border-amber-500/20 rounded-xl p-4">
                      <Suspense fallback={<FormSkeleton />}>
...
// Memoize Services component to prevent unnecessary re-renders
const ServicesWithBoundary = withErrorBoundary(React.memo(Services));
ServicesWithBoundary.displayName = 'Services';
