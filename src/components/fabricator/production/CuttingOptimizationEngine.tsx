...
import type { FinalVerificationData } from '../safety/FinalVerificationModal';
import { SafetyVerificationFlow } from '../safety/SafetyVerificationFlow';
...
export const CuttingOptimizationEngine: React.FC<CuttingOptimizationEngineProps> = ({
  ...
  const [isCuttingPlansCollapsed, setIsCuttingPlansCollapsed] = useState(false);
  const [isCostBreakdownCollapsed, setIsCostBreakdownCollapsed] = useState(false);
  const [isExportCollapsed, setIsExportCollapsed] = useState(false);
  const [isGCodeCollapsed, setIsGCodeCollapsed] = useState(false);
  ...
  // Wrapper to show safety verification flow before generating G-code
  ...
  // Handle safety verification completion for G-code
  const handleSafetyVerificationComplete = async (verificationData: FinalVerificationData) => {
    if (!optimization || !project) return;
    ...
    try {
      ...
      // Update safety log with final G-code hash
      const gcodeHashAfter = await generateGCodeHash(gCodeString);
      await SafetyLoggingService.updateGCodeHashAfter(verificationData.jobId, gcodeHashAfter);
  ...
  // Helper function to generate cryptographically secure G-code hash (SHA-256)
  const generateGCodeHash = async (gcode: string): Promise<string> => {
    try {
      ...
      console.error('Error generating G-code hash:', error);
  ...
  const handleSendToMachine = async () => {
    ...
    try {
      ...
      setExportSuccess(true);
      toast.success('G-code Sent to Machine', {
        description: `G-code has been successfully sent to ${selectedMachine}.`,
        duration: 4000,
      });
    } catch (error) {
    ...
    return (
      <Card className="bg-slate-800/50 border-slate-700/50">
        <CardContent className="p-8 text-center">
          ...
          <h3 className="typography-h3 text-lg mb-2">Generating Cutting Plan</h3>
          <p className="text-slate-400">AI is optimizing your material usage...</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className="bg-slate-800/50 border-slate-700/50">
        <CardContent className="p-8 text-center">
          ...
          <h3 className="typography-h3 text-lg mb-2">No Optimization Data</h3>
          <p className="text-slate-400">Complete the design phase to generate cutting optimization.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-slate-800/50 border-slate-700/50">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-sm text-slate-400">Production Time</div>
          </CardContent>
        </Card>
      ...
      {/* Bar Drawings - Auto-generated (Collapsible) */}
      <Card className="bg-slate-800/50 border-slate-700/50">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsBarDrawingsCollapsed(!isBarDrawingsCollapsed)}
              className="h-6 w-6 p-0 text-amber-400 hover:text-amber-300"
            >
              {isBarDrawingsCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
            </Button>
          </div>
        </CardHeader>
        {!isBarDrawingsCollapsed && (
          <CardContent>
            <BarDrawing
              cuttingPlans={optimizedPlans}
              profiles={profiles}
              projectName={project?.orderNumber}
              orderNumber={project?.orderNumber}
              onExportPDF={handleExportCuttingReport}
              onExportDXF={() => {
                // TODO: Implement DXF export for bar drawings
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsCuttingPlansCollapsed(!isCuttingPlansCollapsed)}
              className="h-6 w-6 p-0 text-amber-400 hover:text-amber-300"
            >
              {isCuttingPlansCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
            </Button>
          </div>
        </CardHeader>
        {!isCuttingPlansCollapsed && (
          <CardContent>
              ...
              {optimizedPlans.map((plan, index) => (
                <div key={index} className="p-4 bg-slate-900 rounded-lg">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h4 className="typography-h4">{plan.profile.name}</h4>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                    {plan.cuts.map((cut, cutIndex) => (
                      <div key={cutIndex} className="p-2 bg-slate-800 rounded text-center">
                        <div className="font-medium">{cut.length}mm</div>
                        <div className="text-slate-400">{cut.angle}°</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsCostBreakdownCollapsed(!isCostBreakdownCollapsed)}
              className="h-6 w-6 p-0 text-amber-400 hover:text-amber-300"
            >
              {isCostBreakdownCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
            </Button>
          </div>
        </CardHeader>
        {!isCostBreakdownCollapsed && (
          <CardContent>
      ...
      {/* Report Export Section (Collapsible) */}
      <Card className="bg-slate-800/50 border-slate-700/50">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
            ...
            <Button
              ...
              className="h-6 w-6 p-0 text-amber-400 hover:text-amber-300"
            >
              {isExportCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
            </Button>
          </div>
        </CardHeader>
        {!isExportCollapsed && (
      ...
      {/* G-code Export Section (Collapsible) */}
      <Card className="bg-slate-800/50 border-slate-700/50">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsGCodeCollapsed(!isGCodeCollapsed)}
              className="h-6 w-6 p-0 text-amber-400 hover:text-amber-300"
            >
              {isGCodeCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
            </Button>
          </div>
        </CardHeader>
        {!isGCodeCollapsed && (
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="typography-label text-sm font-medium">Select Yilmaz Machine:</label>
              <select
                value={selectedMachine}
                onChange={(e) => setSelectedMachine(e.target.value as YilmazMachineModel)}
                className="w-full p-2 bg-slate-900 border border-slate-700/50 rounded-md text-slate-200"
              >
            ...
            {gCodePreview && (
              <div className="mt-4">
                <label className="typography-label text-sm font-medium mb-2 block">G-code Preview:</label>
      ...
      <Card className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border-amber-500/30">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            <div>
              <label className="typography-label text-sm font-medium text-gray-300 mb-2 block">
                Machine: <span className="text-amber-400 font-bold">ALM 6510</span>
              </label>
      ...
      {/* MANDATORY 3-Step Safety Verification Flow */}
      {project && optimization && pendingAction === 'gcode' && (
        <SafetyVerificationFlow
          open={showProductionPreview}
          onOpenChange={setShowProductionPreview}
          onComplete={handleSafetyVerificationComplete}
          components={project.components || []}
          profiles={profiles || []}
          optimizationResult={optimization}
          jobId={project.id || `job-${Date.now()}`}
          machineType={selectedMachine}
          gcodePreview={gCodePreview || undefined}
        />
      )}

      {/* Fallback: Production Preview Dialog for non-G-code actions (report, ALM6510) */}
      {project && pendingAction !== 'gcode' && (
