-- Migration 029: Final Fix for Security Definer Views
-- This migration aggressively fixes views that are being detected as SECURITY DEFINER
-- ============================================================================
-- The issue: Views are being detected as having SECURITY DEFINER property
-- Solution: Drop everything (functions, views, materialized views) and recreate as proper views
-- ============================================================================
...
IF has_material_remnants THEN -- Recreate as a simple view (views are SECURITY INVOKER by default)
...
ELSIF has_ml_snapshots THEN CREATE VIEW public.ml_training_data_view AS
...
DO $$ BEGIN -- Aggressively drop everything
...
) THEN -- Recreate as a simple view
...
IF has_calibration_analytics THEN -- Recreate as a simple view
...
ELSIF has_profile_calibrations THEN CREATE VIEW public.calibration_patterns AS
SELECT profile_id,
    joint_type,
    profile_width_mm,
    profile_height_mm,
...
CREATE VIEW public.erp_transaction_summary AS
SELECT q.user_id,
...
END $$;
-- Success message
SELECT 'Migration 029 completed: Security Definer Views fixed (aggressive cleanup)' as message;