...
-- Authoritative initial schema migrated from database-schema.sql
...
DO $$ BEGIN CREATE TYPE user_role AS ENUM ('customer', 'admin', 'sales_rep', 'technician');
EXCEPTION
...
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE product_category AS ENUM (
	'machine',
	'spare_part',
	'raw_material',
	'tool',
...
WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN CREATE TYPE sector_type AS ENUM ('ALUMINIUM', 'UPVC', 'STEEL', 'GLASS', 'GENERAL');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS public.profiles (
	...
	-- User preferences like language, currency, etc.
...
CREATE TABLE IF NOT EXISTS public.products (
	...
	-- Array of compatible machine SKUs
	-- Media
	image_urls TEXT [],
	video_urls TEXT [],
	document_urls TEXT [],
...
CREATE INDEX IF NOT EXISTS idx_products_search_ar ON public.products USING gin(
	to_tsvector(
		'arabic',
		name_ar || ' ' || COALESCE(description_ar, '')
	)
);
CREATE INDEX IF NOT EXISTS idx_products_search_en ON public.products USING gin(
	to_tsvector(
		'english',
...
CREATE TABLE IF NOT EXISTS public.orders (
	...
	-- Optional: if order came from quote
...
CREATE TABLE IF NOT EXISTS public.notifications (
	...
	type TEXT NOT NULL,
	-- 'order', 'quote', 'product', 'system'
	reference_id UUID,
	-- ID of related order, quote, etc.
...
UPDATE ON public.quotes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
UPDATE ON public.product_reviews FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE OR REPLACE FUNCTION set_quote_number() RETURNS TRIGGER AS $$ BEGIN IF NEW.quote_number IS NULL
	OR NEW.quote_number = '' THEN NEW.quote_number := generate_quote_number();
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql
SET search_path = public;
CREATE OR REPLACE FUNCTION set_order_number() RETURNS TRIGGER AS $$ BEGIN IF NEW.order_number IS NULL
	OR NEW.order_number = '' THEN NEW.order_number := generate_order_number();
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql
...
CREATE TRIGGER set_quote_number_trigger BEFORE
INSERT ON public.quotes FOR EACH ROW EXECUTE FUNCTION set_quote_number();
DROP TRIGGER IF EXISTS set_order_number_trigger ON public.orders;
CREATE TRIGGER set_order_number_trigger BEFORE
INSERT ON public.orders FOR EACH ROW EXECUTE FUNCTION set_order_number();
...
CREATE POLICY "Anyone can view active categories" ON public.categories FOR
SELECT USING (is_active = true);
DROP POLICY IF EXISTS "Anyone can view approved reviews" ON public.product_reviews;
CREATE POLICY "Anyone can view approved reviews" ON public.product_reviews FOR
SELECT USING (is_approved = true);
-- 22. Create RLS policies for user-specific data
DROP POLICY IF EXISTS "Users can view their own quotes" ON public.quotes;
CREATE POLICY "Users can view their own quotes" ON public.quotes FOR
SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can create their own quotes" ON public.quotes;
CREATE POLICY "Users can create their own quotes" ON public.quotes FOR
...
CREATE POLICY "Users can create their own orders" ON public.orders FOR
INSERT WITH CHECK (user_id = auth.uid());
...
VALUES (
	...
	('أدوات', 'Tools', 'tools', 4),
	('إكسسوارات', 'Accessories', 'accessories', 5) ON CONFLICT (slug) DO NOTHING;
-- 25. Insert sample subcategories
INSERT INTO public.categories (name_ar, name_en, slug, parent_id, sort_order)
VALUES (
		'آلات CNC',
		'CNC Machines',
		'cnc-machines',
		(
			...
			WHERE slug = 'industrial-machines'
		),
		1
	),
	(
...
SELECT p.id,
	...
	p.is_featured,
	CASE
		WHEN lang = 'ar' THEN ts_rank(
			to_tsvector(
				'arabic',
				p.name_ar || ' ' || COALESCE(p.description_ar, '')
			),
		...
		ELSE ts_rank(
			to_tsvector(
				'english',
				p.name_en || ' ' || COALESCE(p.description_en, '')
			),
...
WHERE p.is_active = true
	...
	AND (
		...
		p.is_featured = true
		OR -- Same brand as current product
		(
			product_id_param IS NOT NULL
			AND p.brand IN (
				SELECT brand
