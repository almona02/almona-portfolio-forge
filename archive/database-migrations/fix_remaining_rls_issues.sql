...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Valid ticket_status enum values:';
    
    -- Show all valid enum values for ticket_status
    FOR r IN (
        SELECT enumlabel 
        FROM pg_enum e 
        JOIN pg_type t ON e.enumtypid = t.oid 
...
-- Drop any existing consolidated policies that might conflict
DROP POLICY IF EXISTS "Admins can manage parts" ON public.parts;
...
-- Note: Since we created a comprehensive "Admins can manage parts" policy for ALL operations,
-- we don't need separate INSERT, UPDATE, DELETE policies for parts
...
-- Check if the table has user_id or author_id column
DO $$
BEGIN
    IF EXISTS (
    ...
    ELSE
        -- Allow technicians to manage all messages if no user column exists
        CREATE POLICY "Technicians can create messages" ON public.ticket_messages
            FOR INSERT TO authenticated
            WITH CHECK (
                EXISTS (
                    SELECT 1 FROM public.profiles 
