...
// Engineered navbar tailored to Fabricator Pro application
// Orange/Red brand theme, workflow-centric, responsive, with live-like UI signals

export interface IndustrialNavbarProps {
  user?: {
    name: string;
    email: string;
  ...
  metrics?: {
    ...
    efficiencyPct?: string; // e.g. "92.5%"
    activeJobs?: number;
  };
}

const IndustrialNavbar_: React.FC<IndustrialNavbarProps> = ({
  user,
  ...
  // Derived UI metrics with sensible defaults
  const derivedMetrics = useMemo(() => ({
    systemStatus: metrics?.systemStatus ?? 'Optimal',
    efficiencyPct: metrics?.efficiencyPct ?? '92.5%',
    activeJobs: metrics?.activeJobs ?? 12,
  }), [metrics]);
  ...
  // Fabrication modules with efficiency badges
  ...
  const StatusBadge: React.FC<{ status: string; efficiency?: string }> = ({ status, efficiency }) => {
    ...
    const config = (statusConfig as Record<string, { color: string; bg: string; border: string }>)[status] || statusConfig.pending;
  ...
  return (
    ...
    >
      {/* Animated Grid Pattern (Orange-themed) */}
      ...
      <div className="container mx-auto px-4 xl:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-6 flex-shrink-0">
            {/* Back to Almona Forge */}
            <Link
              to="/"
              className="hidden sm:flex items-center gap-2 px-2 py-1.5 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-lg transition-all duration-300 group"
            >
              <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
              <span className="text-xs font-medium">Back to Almona Forge</span>
            </Link>
            ...
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-400 tracking-widest">AI WORKFLOW v4.0</span>
              </div>
            </Link>
            ...
            <div className="hidden lg:flex items-center gap-1">
              {workflowStages.map((stage, index) => (
                <React.Fragment key={stage.id}>
                  ...
                  {index < workflowStages.length - 1 && <div className="w-4 h-px bg-gray-600 mx-1" />}
                </React.Fragment>
              ))}
            </div>
          </div>
          ...
          <div className="hidden xl:flex items-center gap-6 flex-1 justify-center">
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-green-400 font-semibold">{derivedMetrics.systemStatus}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-orange-400 rounded-full animate-pulse" />
                <span className="text-gray-300">Efficiency:</span>
                <span className="text-orange-400 font-semibold">{derivedMetrics.efficiencyPct}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-red-400 rounded-full animate-pulse" />
                <span className="text-gray-300">Active Jobs:</span>
                <span className="text-red-400 font-semibold">{derivedMetrics.activeJobs}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <motion.div className="relative">
              <IndustrialButton onClick={() => setActiveMenu(activeMenu === 'modules' ? null : 'modules')}>
                <CircuitBoard className="w-4 h-4" />
                <span className="hidden lg:inline">Modules</span>
                <ChevronDown
                  className={`w-4 h-4 transition-transform ${activeMenu === 'modules' ? 'rotate-180' : ''}`}
                />
              </IndustrialButton>
                    ...
                    <div className="p-4">
                      ...
                      <div className="space-y-2">
                        {fabricationModules.map((module) => (
                          ...
                          >
                            <div className="text-orange-400 group-hover:scale-110 transition-transform">{module.icon}</div>
              ...
              <AnimatePresence>
                {activeMenu === 'user' && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    className="absolute top-full right-0 mt-2 w-56 bg-gray-800/95 backdrop-blur-xl border border-orange-500/30 rounded-xl shadow-2xl shadow-orange-500/20 overflow-hidden z-50"
                  >
                    <div className="p-2">
                      <div className="px-3 py-2 border-b border-gray-700">
                        ...
                        <div className="text-xs mt-1 text-orange-400 font-semibold uppercase tracking-wider">
                          {user?.role || 'operator'}
                        </div>
                      </div>
              ...
              <div className="space-y-2">
                {fabricationModules.map((module) => (
                  <Link
                    key={module.name}
                    to={module.path}
                    className="flex items-center gap-3 p-3 rounded-lg bg-gray-800/50 border border-gray-700 hover:border-orange-500/30 transition-all duration-200"
                    onClick={() => setMobileOpen(false)}
                  >
                    <div className="text-orange-400">{module.icon}</div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <div className="text-white text-sm truncate">{module.name}</div>
                        <StatusBadge status={module.status} efficiency={module.efficiency} />
                      </div>
                      <div className="text-gray-400 text-xs truncate">{module.description}</div>
                    </div>
                  </Link>
                ))}
              </div>
...
export default IndustrialNavbar_;
