...
class QuoteTemplatesRepository:
    """Persistence layer for quote templates."""
    ...
    def insert_template(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resp = self._db.table("quote_templates").insert(data).execute()
        rows = getattr(resp, "data", []) or []
        if not rows:
            raise RuntimeError("Failed to create quote template")
        return rows[0]
    ...
    def get_template_by_id(
        ...
        query = (
            self._db.table("quote_templates")
            .select("*")
    ...
    def list_templates(
        ...
        if user_id:
            ...
            query = self._db.table("quote_templates").select("*").eq("is_public", True)
