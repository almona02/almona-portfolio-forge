...
class ReportTemplatesRepository:
    """Persistence layer for report templates."""
    ...
    def insert_template(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resp = self._db.table("report_templates").insert(data).execute()
    ...
    def get_template_by_id(
        ...
        query = (
            self._db.table("report_templates")
            .select("*")
        ...
        if user_id:
            ...
            if filters:
                ...
                if filters.get("search"):
                    # Search on name/description (handled in application logic)
                    pass
    ...
    def update_template_fields(
        ...
        # RLS policies handle deleted_at filtering and user ownership
        resp = (
            self._db.table("report_templates")
            .update(update)
            .eq("id", str(template_id))
    ...
    def delete_template(self, template_id: UUID) -> bool:
        ...
        deleted_at = datetime.now(timezone.utc).isoformat().replace(
            "+00:00", "Z"
        )
        (
            self._db.table("report_templates")
