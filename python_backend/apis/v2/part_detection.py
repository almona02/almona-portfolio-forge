...
from flask_limiter.util import (  # type: ignore[reportMissingImports]
    get_remote_address,
)

from ai_services.part_detection.v2.inference import (
    run_inference_v2,
    batch_inference,
)
from ai_services.part_detection.v2.model import PartDetectionModelV2

part_detection_bp = Blueprint(
    'part_detection_v2',
    __name__,
    url_prefix='/api/v2',
)
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            ...
            confidence = _sanitize_confidence(
                request.form.get('confidence', '0.7')
            )
...
def model_info():
    ...
    return jsonify({'success': True, 'data': model.get_model_info()})
