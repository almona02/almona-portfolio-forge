...
from services.industry_watchdog import IndustryWatchdog, MarketAlert, IndustryArticle
...
async def get_latest_trends(
    ...
    timeframe: Optional[str] = Query("last_30_days", description="Timeframe: last_7_days, last_30_days, last_90_days"),
    relevance_threshold: Optional[float] = Query(0.5, description="Minimum relevance (0-1)"),
    ...
    try:
        ...
        if categories:
            ...
            filtered = [
                ...
                if any(cat in [c.lower() for c in article.categories] for cat in category_list)
            ]
...
async def get_morning_brief(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching morning brief: {str(e)}")
    ...
    try:
        ...
        matching = [
            ...
            if keyword_lower in article.title.lower() or
               ...
               any(keyword_lower in kw.lower() for kw in article.keywords)
        ]
