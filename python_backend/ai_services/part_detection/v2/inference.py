...
def run_inference_v2(
    image_path: str, confidence_threshold: float = 0.7
) -> Dict[str, any]:
    """
    Enhanced inference with confidence filtering and batch processing.
    ...
    top_pred = (
        max(results.items(), key=lambda x: x[1]) if results else None
    )
...
def batch_inference(
    image_paths: List[str], confidence_threshold: float = 0.7
) -> List[Dict[str, any]]:
