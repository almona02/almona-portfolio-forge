...
ydt_agent_path = Path(__file__).parent.parent.parent / "ai_agents" / "ydt_agent"
sys.path.append(str(ydt_agent_path))
    ...
    try:
        ...
        if not knowledge_base_path.exists():
            ...
            for alt_path in alt_paths:
                if alt_path.exists():
                    logger.info(
                        f"Found knowledge base at alternative path: " f"{alt_path}"
                    )
        ...
        if knowledge_base_path.exists():
            ...
            logger.error(
                f"❌ Knowledge base path does not exist: " f"{knowledge_base_path}"
            )
