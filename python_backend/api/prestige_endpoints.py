...
try:
    from ydt_chatbot_engine import YDTChatbotEngine, Language, Persona, ChatContext
    from gcode_ydt_integration import YDTGCodeEnhancer, GCodeYDTValidator
    from arabic_support import translate_term, format_arabic_response, detect_arabic
...
class ChatRequest(BaseModel):
    message: str
    persona: str = "professor"  # professor/doctor/tour-guide/code-master/nervous-system
...
async def chat_endpoint(request: ChatRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Validate persona
        valid_personas = ["professor", "doctor", "tour-guide", "tour_guide", "code-master", "code_master", "nervous-system", "nervous_system"]
        persona_map = {
        ...
        language_enum = lang_map.get(request.language, Language.ENGLISH if Language else "en")
        ...
        return {
            ...
            "data": {
                ...
                "extras": {
                    ...
                    "has_exercises": response.get("extras", {}).get("hasExercise", False),
                    "difficulty_level": response.get("extras", {}).get("difficulty", "intermediate")
                }
            },
            "metadata": {
                "timestamp": datetime.now().isoformat(),
...
async def get_learning_modules(language: str = "en"):
    ...
    try:
        ...
        try:
            from university_curriculum import CURRICULUM_STRUCTURE, get_module_progress, Module
        except ImportError:
    ...
    try:
        if not ydt_engine:
            ...
            return {
                "success": True,
                "data": {
                    "diagnosis": "Based on symptoms, check electrical connections and component status.",
                    "confidence": 85,
                    "probable_causes": ["Electrical connection issue", "Component failure"],
                    "immediate_actions": ["Check power supply", "Inspect connections"],
                    "repair_steps": ["1. Power off machine", "2. Check connections", "3. Test components"],
        ...
        return {
            ...
            "data": {
                ...
                "immediate_actions": chat_response.suggested_actions or ["Check connections", "Review manual"],
                "repair_steps": ["1. Power off", "2. Inspect", "3. Test", "4. Repair"],
                "required_tools": ["Multimeter", "Screwdriver"],
                "required_parts": [],
                "estimated_time": "30 minutes",
                "urgency": "medium",
...
async def get_machine_capabilities():
    ...
    return {
        ...
        "data": {
            ...
            "capabilities": {
                "operations": [
                    "Cutting", "Drilling", "Milling", "Tapping", "Grooving",
                    "Notching", "Countersinking", "Threading", "Chamfering"
                ],
                "materials": [
                    "Aluminium", "UPVC", "Composite", "Steel (thin-walled)",
                    "Brass", "Copper", "Plastics"
                ],
                "applications": [
                    "Windows", "Doors", "Curtain Walls", "Facades",
                    "Solar Panel Frames", "Display Systems", "Furniture",
...
async def log_interaction(request: ChatRequest, response: Dict, response_time: float):
    ...
    log_entry = {
        ...
        "response_preview": (response.get("content", response.get("response", ""))[:100] + "...") if len(response.get("content", response.get("response", ""))) > 100 else response.get("content", response.get("response", "")),
        "confidence": response.get("confidence", 95),
        "response_time": response_time,
        "knowledge_sources_used": response.get("sources", response.get("knowledge_sources", []))[:3]
    }
