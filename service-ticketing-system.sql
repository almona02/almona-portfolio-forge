-- Almona Service Ticketing System Integration
-- This SQL adds a professional service ticketing system to the existing Supabase schema

-- 1. Create additional ENUM types for the ticketing system (with IF NOT EXISTS to prevent conflicts)
DO $$ BEGIN
...
-- 2. Create SLA configuration table (with IF NOT EXISTS)
...
-- 3. Create service_tickets table (enhanced version)
...
-- 4. Create ticket_messages table (enhanced version)
...
-- 5. Create ticket_assignments_history table for audit trail
...
-- 6. Create ticket_escalations table
...
-- 7. Insert default SLA configurations (with conflict handling)
...
-- 9. Create function to auto-assign tickets based on type
...
-- 10. Create function to calculate SLA due dates
CREATE OR REPLACE FUNCTION calculate_sla_dates(ticket_priority_param ticket_priority, ticket_type_param ticket_type, created_at_param TIMESTAMPTZ)
...
-- 11. Create function to handle spare parts requests
CREATE OR REPLACE FUNCTION create_spare_parts_quote(ticket_id_param UUID, spare_parts_details_param JSONB)
RETURNS UUID AS $$
DECLARE
    ticket_record RECORD;
    new_quote_id UUID;
...
-- 13. Create trigger function for ticket updates
...
-- 14. Create trigger function for message creation
...
BEGIN
    ...
    IF NEW.message_type = 'spare_parts_request' AND NEW.spare_parts_details IS NOT NULL THEN
        quote_id := create_spare_parts_quote(NEW.ticket_id, NEW.spare_parts_details);
...
-- 15. Create triggers (with safe creation)
...
CREATE TRIGGER handle_ticket_update_trigger
    ...
    FOR EACH ROW EXECUTE FUNCTION handle_ticket_update();
...
-- 16. Create indexes for performance (with IF NOT EXISTS)
...
-- 19. Create useful views for reporting
...
-- Success message
SELECT 'Service Ticketing System Created Successfully!' as message;
